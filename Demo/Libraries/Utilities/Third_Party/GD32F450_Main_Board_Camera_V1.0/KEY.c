...
{
    uint8_t value = 0;
    if(SWITCH_ON == SWITCH_Get_Input(SWITCH1)) value |= 1;			//up
    if(SWITCH_ON == SWITCH_Get_Input(SWITCH2)) value |= (1<<1);		//left  
    if(SWITCH_ON == SWITCH_Get_Input(SWITCH3)) value |= (1<<2);	//right
    if(SWITCH_ON == SWITCH_Get_Input(SWITCH4)) value |= (1<<3);		//down
    return value;   
}
