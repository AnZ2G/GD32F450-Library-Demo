...
void PWM_Config(uint16_t FrequencyDivision, uint16_t ReloadValue)
{
	...
	gpio_mode_set(GPIOA, GPIO_MODE_AF, GPIO_PUPD_PULLUP, GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10 | GPIO_PIN_11);
	gpio_output_options_set(GPIOA, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_9 | GPIO_PIN_9 | GPIO_PIN_10 | GPIO_PIN_11);
    ...
    timer_initpara.period            = ReloadValue - 1;								//写入自动重装载值
    ...
    /* 配置舵机引脚 */
    gpio_output_init(GPIOA, GPIO_MODE_AF, GPIO_PUPD_NONE, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_2);
    gpio_af_set(GPIOA, GPIO_AF_3, GPIO_PIN_2);
    pwm_init(RCU_TIMER8, TIMER8, TIMER_CH_0, 199, 19999, 1400);
...
//-------------------------------------------------------------------------------------------------------------------
// @brief		更新PWM通道占空比
// @param		ch				更新的PWM通道：0,1,2,3
// @param		dutycycle		占空比（千分之）
// @return		void
// Sample usage:				pwm_duty_updata(0,500);//更新PWM通道0的占空比为50％(500/1000)
//-------------------------------------------------------------------------------------------------------------------
void PWM_Duty_Updata(uint8_t ch, uint16_t dutycycle)
{
    uint16_t output_value,channal;
    output_value = dutycycle*Reload/1000;
    if(ch==0) channal = TIMER_CH_0;
    if(ch==1) channal = TIMER_CH_1;
    if(ch==2) channal = TIMER_CH_3;
    if(ch==3) channal = TIMER_CH_2;
    timer_channel_output_pulse_value_config(TIMER0,channal,output_value);
}
...
// @return		void
// Sample usage:				steer_updata(1350);//更新舵机高电平时间为1235us
//-------------------------------------------------------------------------------------------------------------------
void Steer_Updata(uint16_t period)
{
    timer_channel_output_pulse_value_config(TIMER8,TIMER_CH_0,period);
}
