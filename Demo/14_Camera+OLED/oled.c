...
void Bmp_display(uint8_t picture_buffer[64][128])
{
    ...
    for(i=0;i<128;i++)
    {
        ...
        for(j=0;j<4;j++)
        {
            if(picture_buffer[j+4][i]>0x20) data_temp += oled_pow(2,j);
        }
        data_temp *= 16;
        for(j=0;j<4;j++)
        {
            if(picture_buffer[j][i]>0x20) data_temp += oled_pow(2,j);
        }
	    i2c_send_reg_byte(0x78, 0x40, data_temp);
    }
    ...
    for(i=0;i<128;i++)
    {
        ...
        for(j=0;j<4;j++)
        {
            if(picture_buffer[j+12][i]>0x20) data_temp += oled_pow(2,j);
        }
        data_temp *= 16;
        for(j=0;j<4;j++)
        {
            if(picture_buffer[j+8][i]>0x20) data_temp += oled_pow(2,j);
        }
	    i2c_send_reg_byte(0x78, 0x40, data_temp);
    }
    ...
    for(i=0;i<128;i++)
    {
        ...
        for(j=0;j<4;j++)
        {
            if(picture_buffer[j+20][i]>0x20) data_temp += oled_pow(2,j);
        }
        data_temp *= 16;
        for(j=0;j<4;j++)
        {
            if(picture_buffer[j+16][i]>0x20) data_temp += oled_pow(2,j);
        }
	    i2c_send_reg_byte(0x78, 0x40, data_temp);
    }
    ...
    for(i=0;i<128;i++)
    {
        ...
        for(j=0;j<4;j++)
        {
            if(picture_buffer[j+28][i]>0x20) data_temp += oled_pow(2,j);
        }
        data_temp *= 16;
        for(j=0;j<4;j++)
        {
            if(picture_buffer[j+24][i]>0x20) data_temp += oled_pow(2,j);
        }
	    i2c_send_reg_byte(0x78, 0x40, data_temp);
    }
    ...
    for(i=0;i<128;i++)
    {
        ...
        for(j=0;j<4;j++)
        {
            if(picture_buffer[j+36][i]>0x20) data_temp += oled_pow(2,j);
        }
        data_temp *= 16;
        for(j=0;j<4;j++)
        {
            if(picture_buffer[j+32][i]>0x20) data_temp += oled_pow(2,j);
        }
	    i2c_send_reg_byte(0x78, 0x40, data_temp);
    }
    ...
    for(i=0;i<128;i++)
    {
        ...
        for(j=0;j<4;j++)
        {
            if(picture_buffer[j+44][i]>0x20) data_temp += oled_pow(2,j);
        }
        data_temp *= 16;
        for(j=0;j<4;j++)
        {
            if(picture_buffer[j+40][i]>0x20) data_temp += oled_pow(2,j);
        }
	    i2c_send_reg_byte(0x78, 0x40, data_temp);
    }
    ...
    for(i=0;i<128;i++)
    {
        ...
        for(j=0;j<4;j++)
        {
            if(picture_buffer[j+52][i]>0x20) data_temp += oled_pow(2,j);
        }
        data_temp *= 16;
        for(j=0;j<4;j++)
        {
            if(picture_buffer[j+48][i]>0x20) data_temp += oled_pow(2,j);
        }
	    i2c_send_reg_byte(0x78, 0x40, data_temp);
    }
    ...
    for(i=0;i<128;i++)
    {
        ...
        for(j=0;j<4;j++)
        {
            if(picture_buffer[j+60][i]>0x20) data_temp += oled_pow(2,j);
        }
        data_temp *= 16;
        for(j=0;j<4;j++)
        {
            if(picture_buffer[j+56][i]>0x20) data_temp += oled_pow(2,j);
        }
	    i2c_send_reg_byte(0x78, 0x40, data_temp);
    }
}
...
{ 	 
  ...
  for(i=0;i<56;i+=8)// 56行 
  {
    ...
    { 
      temp=0;
      if(Img[0+i][j]>=120) temp|=1;
      if(Img[1+i][j]>=120) temp|=2;
      if(Img[2+i][j]>=120) temp|=4;
      if(Img[3+i][j]>=120) temp|=8;
      if(Img[4+i][j]>=120) temp|=0x10;
      if(Img[5+i][j]>=120) temp|=0x20;
      if(Img[6+i][j]>=120) temp|=0x40;
      if(Img[7+i][j]>=120) temp|=0x80;
