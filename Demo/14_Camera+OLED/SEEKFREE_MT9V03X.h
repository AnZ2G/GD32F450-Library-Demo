...
//引脚配置
//--------------------------------------------------------------------------------------------------
#define MT9V03X_COF_UART		USART0										// 配置摄像头所使用到的串口
...
extern uint8_t    mt9v03x_finish_flag;//一场图像采集完成标志位
extern uint8_t    mt9v03x_image[MT9V03X_H][MT9V03X_W];

void set_config(uint32_t usart_periph, int16_t buff[CONFIG_FINISH-1][2]);
void get_config(uint32_t usart_periph, int16_t buff[CONFIG_FINISH-1][2]);
uint16_t get_version(uint32_t usart_periph);
uint16_t set_exposure_time(uint32_t usart_periph, uint16_t light);

void mt9v03x_init(void);
void mt9v03x_vsync(void);
void mt9v03x_dma(void);
void mt9v03x_uart_callback(void);
void seekfree_sendimg_03x(uint32_t usart_periph, uint8_t *image, uint16_t width, uint16_t height);
