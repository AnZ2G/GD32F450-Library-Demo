...
//-------------------------------------------------------------------------------------------------------------------
void set_config(uint32_t uartn, int16_t buff[CONFIG_FINISH-1][2])
{
	uint16_t temp, i;
...
// Sample usage:				调用该函数前请先初始化串口
//-------------------------------------------------------------------------------------------------------------------
void get_config(uint32_t uartn, int16_t buff[CONFIG_FINISH-1][2])
{
	uint16_t temp, i;
	uint8_t  send_buffer[4];
...
// Sample usage:				调用该函数前请先初始化串口
//-------------------------------------------------------------------------------------------------------------------
uint16_t get_version(uint32_t uartn)
{
	uint16_t temp;
	uint8_t  send_buffer[4];
	send_buffer[0] = 0xA5;
...
// Sample usage:				调用该函数前请先初始化串口
//-------------------------------------------------------------------------------------------------------------------
uint16_t set_exposure_time(uint32_t uartn, uint16_t light)
{
	uint16_t temp;
	uint8_t  send_buffer[4];
...
// Sample usage:				调用该函数前请先初始化串口
//-------------------------------------------------------------------------------------------------------------------
uint16_t set_mt9v03x_reg(uint32_t uartn, uint8_t addr, uint16_t data)
{
	uint16_t temp;
	uint8_t  send_buffer[4];
...
void mt9v03x_exti_init(void)
{
    ...
    gpio_mode_set(GPIOB, GPIO_MODE_INPUT, GPIO_PUPD_PULLDOWN, GPIO_PIN_9|GPIO_PIN_8);
...
{
    gpio_mode_set(GPIOG, GPIO_MODE_INPUT, GPIO_PUPD_NONE, GPIO_PIN_8|GPIO_PIN_9|GPIO_PIN_10|GPIO_PIN_11|GPIO_PIN_12|GPIO_PIN_13|GPIO_PIN_14|GPIO_PIN_15);
	//DMA1总线初始化
//	RCC_AHBPeriphClockCmd(RCC_AHBPeriph_DMA1, ENABLE);
	    
	//DMA摄像头初始化
	camera_dma_init((uint32_t)(volatile uint32_t *)(uint32_t)((GPIOG) + 0x10U +1), (uint32_t)camera_buffer_addr, MT9V03X_H*MT9V03X_W);
	//中断配置
...
//	camera_type = CAMERA_GRAYSCALE;//设置连接摄像头类型
    ...
    /* 配置TXD和RXD引脚以及串口 */
    uart_tx_init(GPIOA, GPIO_PIN_15, RCU_GPIOA);
    uart_rx_init(GPIOB, GPIO_PIN_3, RCU_GPIOB);
    uart_init(RCU_USART0, USART0, 9600U);  
 ...
 printf("3\n");
    ...
    dcmi_init();
    //初始化dcmi的dma通道
    dcmi_dma_init((uint32_t)camera_buffer_addr,MT9V03X_H*MT9V03X_W/4);//DCMI DMA配置
...
//-------------------------------------------------------------------------------------------------------------------
void mt9v03x_uart_display(void)
{
    usart_data_transmit(USART2, 1);														
    while(RESET == usart_flag_get(USART2, USART_FLAG_TBE));
    usart_data_transmit(USART2, ~1);														
    while(RESET == usart_flag_get(USART2, USART_FLAG_TBE));	

    uart_putbuff(USART2, mt9v03x_image[0], 8192);
...
//-------------------------------------------------------------------------------------------------------------------
void mt9v03x_oled_display(void)
{
    flash_buffer_translate(mt9v03x_image[0],Img[0],0,0,8192);
    OLED_camera();
}
