...
{
//    LED_Init();  //初始化所有LED
    KEY_Init(KEY_UP);  //初始化所有按键
    KEY_Init(KEY_DOWN);  //初始化所有按键
    KEY_Init(KEY_LEFT);  //初始化所有按键
    KEY_Init(KEY_RIGHT);  //初始化所有按键
    KEY_Init(KEY_SET);  //初始化所有按键
    KEY_Init(KEY_BACK);  //初始化所有按键
//	  switch_init();
}
/*************************************************
函数名称: switch_init()
...
void HCI_init(void)
{
//		FLASH_InitConfig();  //初始化flash
    OLED_Init();  //OLED初始化
    delay_1ms(200);  //初始化后必要的延时
    Clr_Screen(8);  //清屏
    led_key_init();  //LED和按键初始化函数（调试用）
    value_init(); 
    switch_init();  //初始化所有拨码开关
}
...
{
//	 FLASH_ReadBuff(0, 0, 0, uc_record_P_value, 8);
//	 FLASH_ReadBuff(0, 0, 16, uc_record_I_value, 8);
//	 FLASH_ReadBuff(0, 0, 32, uc_record_D_value, 8);
//	 FLASH_ReadBuff(0, 0, 48, uc_record_PD_P, 8);
//	 FLASH_ReadBuff(0, 0, 64, uc_record_b_value, 8);
//	 FLASH_ReadBuff(0, 0, 80, uc_record_PD_D, 8);
//	 FLASH_ReadBuff(0, 0, 96, uc_record_mid_speed, 8);
//	 FLASH_ReadBuff(0, 0, 112, uc_record_high_speed, 8);
//	 FLASH_ReadBuff(0, 0, 128, uc_record_low_speed, 8);
//	 FLASH_ReadBuff(0, 0, 144, uc_record_s_a_value, 8);
//	 FLASH_ReadBuff(0, 0, 160, uc_record_s_b_value, 8);
//	 FLASH_ReadBuff(0, 0, 176, uc_record_s_c_value, 8);
//	 FLASH_ReadBuff(0, 0, 192, uc_record_Set_speed_pwm, 8);

//	 FLASH_ReadBuff(0, 0, 208, uc_record_res2, 8);
//	 FLASH_ReadBuff(0, 0, 224, uc_record_res3, 8);
//	 FLASH_ReadBuff(0, 0, 240, uc_record_res4, 8);		 
//	 FLASH_ReadBuff(0, 0, 256, uc_record_res5, 8);
//   FLASH_ReadBuff(0, 0, 272, uc_record_res6, 8);

		flash_ReadSector_uint8_t(2, 0, read_buffer, 144);
