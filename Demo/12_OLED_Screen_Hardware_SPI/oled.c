...
void OLED_Init_GPIO(void)
{
    ...
    gpio_output_init(GPIOB, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_12|GPIO_PIN_14);  
    gpio_output_init(GPIOB, GPIO_MODE_AF, GPIO_PUPD_PULLUP, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_13|GPIO_PIN_15);
...
void OLED_show_ascii(uint8_t ascii_buffer[16],uint8_t line_num)
{
    ...
    for(i=0;i<8;i++)
    {
        OLED_buffer[line_num*16+i] = ascii_buffer[i];
    }
    for(i=0;i<8;i++)
    {
        OLED_buffer[line_num*16+128+i] = ascii_buffer[i+8];
    }
}
...
void OLED_show_ascii_half(uint8_t ascii_buffer[16],uint8_t line_num)
{
    ...
    for(i=0;i<8;i++)
    {
        OLED_buffer[line_num*16+8+i] = ascii_buffer[i];
    }
    for(i=0;i<8;i++)
    {
        OLED_buffer[line_num*16+128+8+i] = ascii_buffer[i+8];
    }
}
...
void OLED_show_chinese(uint8_t chinese_buffer[32],uint8_t line_num)
{
    ...
    for(i=0;i<16;i++)
    {
        OLED_buffer[line_num*16+i] = chinese_buffer[i];
    }
    for(i=0;i<16;i++)
    {
        OLED_buffer[line_num*16+128+i] = chinese_buffer[i+16];
    }
}
